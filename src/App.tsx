...
import { Cake, Cupcake } from "./models/cake";
...
const ORBIT_INITIAL_RADIUS = 4.2;
...
const TYPED_LINES = [
  ...
  " ... ",
  " > aku buat digital cake 🎂 buat kamuu ehehehe",
  " ... ",
  " > make a wish... (ﾉ◕ヮ◕)ﾉ*:･ﾟ✧",
];
...
const CARD_ZOOM_MAX = 2.1;
const CARD_ZOOM_STEP = 0.15;
...
/**
 * Main animated 3D scene containing cake, table, candles, and picture frames
 */
function AnimatedScene({
  ...
  // Emit environment progress changes
...
/**
 * Orbit controls with configured constraints
 */
function ConfiguredOrbitControls() {
  const controlsRef = useRef<OrbitControlsImpl>(null);
  const camera = fiber.useThree((state: fiber.RootState) => state.camera);
...
/**
 * Controller for environment background intensity
 */
function EnvironmentBackgroundController({
  intensity,
}: EnvironmentBackgroundControllerProps) {
  const scene = fiber.useThree((state: fiber.RootState) => state.scene);
...
export default function App() {
  ...
  const handleCardToggle = useCallback((id: string) => {
    const initialZoom = isCompactViewport ? 0.85 : 1;
    setActiveCardId((current) => {
      const next = current === id ? null : id;
      if (next !== current) {
        setCardZoom(initialZoom);
      }
      return next;
    });
  }, [isCompactViewport]);

  const zoomMin = isCompactViewport ? 0.45 : CARD_ZOOM_MIN;
  const zoomMax = isCompactViewport ? 1.9 : CARD_ZOOM_MAX;
  const zoomStep = isCompactViewport ? 0.1 : CARD_ZOOM_STEP;

  const handleZoomIn = useCallback(() => {
    setCardZoom((current) =>
      clamp(current + zoomStep, zoomMin, zoomMax)
    );
  }, [zoomMax, zoomMin, zoomStep]);
  ...
  }, [activeCardId, zoomMax, zoomMin, zoomStep]);
    ...
    <div className="App">
      ...
      {!hasStarted && (
        <div className="start-overlay">
          <div className="start-title">🎂 A Gift For You 🎂</div>
      ...
      {/* Card Zoom Controls */}
      {activeCardId !== null && (
        <div className="card-zoom-controls" aria-label="Card zoom controls">
      ...
      >
        <Suspense fallback={null}>
          ...
          <Environment
            ...
            backgroundIntensity={0.08 * environmentProgress}
          />
      ...
      {hasAnimationCompleted && !isCandleLit && (
        <div
          ...
          style={{ pointerEvents: "auto", bottom: "5rem" }}
        >
          <button
